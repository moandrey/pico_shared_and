...
#define MAX_REPORT 4
    ...
    void tuh_hid_mount_cb(uint8_t dev_addr, uint8_t instance, uint8_t const *desc_report, uint16_t desc_len)
    {
        ...
        if (vid == 0x0079) {
            ...
            if (pid == 0x0126) {
                printf(">>> SENDING MAGIC WAKE-UP PACKET TO TGZ-850M <<<\n");
                static uint8_t magic_packet[4] = {0x01, 0x00, 0x00, 0x00}; 
                tuh_hid_send_report(dev_addr, instance, 0, magic_packet, sizeof(magic_packet));
            }
        }
        // ==========================================
...
case HID_USAGE_DESKTOP_JOYSTICK:
                {
                    ...
                    if (vid == 0x0079 && pid == 0x0126) {
                        ...
                        // Стандартная обработка для остальных джойстиков
